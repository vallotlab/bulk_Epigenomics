...
echo "singularity  exec --bind ${bind_directory}:${bind_directory} --bind ${output_dir}:/mnt/ $image /bin/bash -c \"/mnt/run_bulk_Epigenomics.sh ${params}\" " # | qsub -l nodes=1:ppn=8,mem=30gb -N $(basename ${output_dir})